...
const char *FunctionNode::getAttributeCString() const{
    int begin, end;
    prototype = this->getPrototypeCString();
    begin = prototype.find("(") + 1;
    end = prototype.find(")");
    attribute = prototype.substr(prototype.find("(") + 1, prototype.find(")") - (prototype.find("(") + 1));
    //attribute = prototype;
